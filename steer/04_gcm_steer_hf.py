...
def _run_steering(args):
    ...
    val_dataset_pairs = gcm.prepare_stego_pairs(data_path, num_samples=args.val_num_samples)
    ...
    heads_file = os.path.join(
        REPO_ROOT, "data",
        f"{model_tag}_top{args.k_pct}_stego_heads_{args.dataset_tag}.json"
    )
    ...
    steering_vectors = compute_stego_steering_vectors(
        model, tokenizer, dataset_pairs, top_heads[:10], gcm.format_stego_prompt
    )

    vec_save_path = os.path.join(
        REPO_ROOT, "data", f"{model_tag}_stego_steering_vectors_{args.dataset_tag}.pt"
    )
    ...
    csv_path = os.path.join(
        REPO_ROOT, "data",
        f"{model_tag}_top{args.k_pct}_stego_steering_results_{args.dataset_tag}.csv"
    )
    ...
    summary = {
        **config_capture,
        ...
        "num_heads_used": len(top_heads[:10]),
        "alpha": args.alpha,
    ...
    summary_path = os.path.join(
        REPO_ROOT, "data",
        f"{model_tag}_steering_summary_{args.dataset_tag}.json"
    )
